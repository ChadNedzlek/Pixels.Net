...
internal class Dispatcher
{
    ...
    public bool IsDispatchRequired => !RuntimeInformation.IsOSPlatform(OSPlatform.Windows) ||
    ...
    private Task<object> SendToDispatcher(Func<object> handler)
    {
        ...
        _queue.Enqueue(new DispatchRecord(handler, c, Stopwatch.StartNew()));
        _readyEvent.Set();
        return c.Task;
    }
    ...
    private void PrepareBackgroundThread()
    {
        ...
        lock (_prepareLock)
        {
            ...
            Logger.Instance.Log(PixelsLogLevel.Info,
                "Initiating background dispatcher thread to handle thread apartments");
